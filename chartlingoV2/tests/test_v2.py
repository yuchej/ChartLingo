...
class ChartLingoV2Tests(unittest.TestCase):
    ...
    def test_mismatch_checklist_stages_decisions_and_clears_resolved_warnings(self):
        index = (ROOT / "index.html").read_text()
        app = (ROOT / "app.js").read_text()
        styles = (ROOT / "styles.css").read_text()
        ...
        self.assertIn(".mismatch-checklist-row.decision-ready", styles)
        self.assertIn("function checklistSuggestion", app)
        self.assertIn("best.confidence>=.72", app)
        self.assertIn("best.confidence>=.35&&best.confidence-second>=.08", app)
        self.assertIn('class="suggested-match"', app)
        self.assertIn(".suggested-match", styles)
