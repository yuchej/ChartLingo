...
- Illustrator exporter 0.8.7 exports directly into any selected folder, including nested folders and paths containing spaces or Unicode characters. It tests write access in that exact folder, generates collision-safe filenames from the Illustrator document name, reads every completed package back, validates its JSON and schema, and only reports success after every expected file is present and verified. Raster photos are embedded as data-URI `<image>` elements, while lines, shapes, logos, and other Illustrator artwork remain editable SVG vectors; the exporter stops instead of flattening the entire artwork when image embedding is invalid. It removes XML-invalid control characters from embedded SVG artwork, while the browser importer repeats this cleanup for packages made with older exporters. It preserves Illustrator font styles with numeric weights from 100–900, separates explicit lines and whitespace-delimited items inside a text frame for better translation matching, keeps a same-size multi-line headline as one object, and rejoins decimal axis fragments such as `8` + `.0`. Tabular exports preserve tab anchors and give the final column a full neighboring column width.
...
- SVG export uses an Illustrator-compatible SVG 1.1 XML header, complete Adobe entity declarations, direct namespace URLs, and final XML validation before saving.
